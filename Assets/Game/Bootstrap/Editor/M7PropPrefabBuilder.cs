...
namespace RaidDemo.Bootstrap.Editor
{
    ...
    public static partial class M7PropPrefabBuilder
    {
        ...
        {
            var model = LoadSourceModel(recipe.Source, recipe.SourceName);
            if (model == null)
            {
                return null;
            }
        ...
        /// <summary>按来源分派到对应的素材解析入口。</summary>
        /// <remarks>
        /// 每个来源的候选目录（正式目录 → 暂存区）都收敛在 <see cref="M7SceneAssetResolver"/> 里，
        /// 构建器只负责声明「这个配方属于哪个包」，避免同一份路径散落在两个文件里。
        /// </remarks>
        private static GameObject LoadSourceModel(SourceKind source, string sourceName)
        {
            switch (source)
            {
                case SourceKind.CityKit:
                    return M7SceneAssetResolver.LoadIndustrialProp(sourceName);
                case SourceKind.FactoryKit:
