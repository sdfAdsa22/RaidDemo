...
namespace RaidDemo.AI
{
    ...
    /// </summary>
    /// <remarks>
    /// <para><b>为什么按速度分档而不是按姿态分档：</b>本项目对标《逃离鸭科夫》，
    ...
    /// </summary>
    /// <remarks>
    /// <para>它由**产生噪音的一方**（当前是玩家移动）发布，由 AI 的感知系统消费。
    ...
    /// </remarks>
    public readonly struct MovementNoiseEvent : IEventEnvelope
    {
        ...
        public MovementNoiseTier Tier { get; }
        ...
        {
            return $"MovementNoiseEvent(来源={SourceId}, 档位={Tier}, 位置={Position})";
        }
    }
    ...
    public static class MovementNoiseRules
    {
        ...
        public static MovementNoiseTier Classify(float speed, float sprintSpeedThreshold, bool isOverloaded)
        {
            if (speed <= MovingSpeedThreshold)
            {
                // 静止优先于超载：站着不动就是静音，超载不会凭空制造声音。
                return MovementNoiseTier.Silent;
            }

            if (isOverloaded)
            {
                return MovementNoiseTier.Overloaded;
            }

            return speed >= sprintSpeedThreshold
                ? MovementNoiseTier.Sprint
                : MovementNoiseTier.Walk;
        }
    }
}
