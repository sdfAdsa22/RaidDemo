...
namespace RaidDemo.Input
{
    ...
    public sealed partial class PlayerInputCollector : MonoBehaviour
    {
        ...
        /// 瞄准解算所用的水平面高度。
        /// </summary>
        /// <remarks>由装配层写入"脚底 + 枪口高度"：准星、弹道与枪口共面之后，
        /// 弹道在屏幕上必然穿过准星（U-95 方案 A）。若传地面高度，就会退回
        /// "弹道贴地、与准星分处两个平面"的老问题。</remarks>
        ...
        /// <summary>
        /// 设置瞄准解算所用的水平面高度（见字段说明：装配层传"脚底 + 枪口高度"）。
        /// </summary>
        /// <remarks>
        /// 把鼠标投影到角色所在的水平面而不是固定的世界零平面，
        /// 是为了让瞄准方向在角色位于不同高度的地形上时依然准确；
        /// 平面再抬到枪口高度之后，准星与弹道共面，屏幕上线与准星不再错开。
