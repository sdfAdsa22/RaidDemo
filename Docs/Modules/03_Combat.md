...
- **"绘制"这条曾经被实现改掉过**：为了修"站在装卸平台上的敌人弹道被画到平台下面"，
  `TracerRenderer` 一度改成"保留真实高度"——结果是弹道画在枪口高度（1.05 米）、
  准星在地面，45° 斜俯视下两者在屏幕上必然错开（本页描述与实现漂移了数周）。
  现在改为**向下探测各自的地面/平台面**（探测遮罩排除单位层）：平地贴地穿过准星、
  平台上仍画在平台面，两个问题同时解决。
