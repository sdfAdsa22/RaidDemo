...
**2026-09-17 修订（负责人反馈的"打到自己 / 开局偏移 / 弹道与准星不共线"）**：

- **"绘制"与"方向"最终统一到同一平面（方案 A）**：这里先后试过三种画法——
  ① "保留真实高度"（起点/终点都正确，但准星在地面，屏幕上与弹道错开）；
  ② "贴地画线"（穿过准星，但起点被压到枪口正下方的地面、终点被压到命中点下方的地面）；
  ③ 三点折线（起点贴住枪口，但终点仍在地面，与命中火花错开）。
  根因始终是**准星在地面、弹道在 1.05 米平面**——两个平面在 45° 斜俯视下必然错开，
  任何投影画法都只能保住其中一环。最终把**瞄准解算平面抬到枪口高度**
  （装配层传"脚底 + MuzzleHeight"，战局与安全屋同一条规则），准星、弹道、枪口三者共面：
  弹道恢复为"枪口 → 终点"的两点**真实直线**，屏幕上自然穿过准星，命中火花就在线端。
  代价：瞄准点的世界坐标沿相机视线前移约 1.05 米（准星屏幕位置不变）。
- **探针新增"跳过射手自己"**：枪口在身体外侧，瞄准点落在角色附近/侧后方时射线会穿过自己的身体；
  自伤原本只在伤害层拦下，开火事件仍带着"命中自己"的终点，表现为弹道停在自己身上、
  子弹也被自己挡住。`IHitProbe.TryRaycastIgnoringTarget` 把射手当作透明继续往后投，
  玩家与 AI 两条开火链路都已接入（细节见 `Docs/12_用户反馈问题记录.md` U-95）。
- **时序**：瞄准解算在"回写玩家最新位置"之前、枪口在武器视图更新之前——开局第一帧会用到默认 (0,0)。
  现在位置回写提前、取枪口后移，瞄准同步统一到战斗更新开头。
...
### 13.11 手持武器的姿态与弹道起点（P-47）

**源模型的坐标约定**：Quaternius Toon Shooter 的枪是 **Z-up**——朝向 +X、上方 +Z。
项目要的是"前 = +Z、上 = +Y"，因此 `M7WeaponPrefabBuilder` 必须做**两步**旋转：

```text
① 绕 Y 轴 -90°：把枪口从 +X 转到 +Z
② 绕枪管轴滚 -90°：把源模型的 +Z（枪的上方）立到 +Y
```

少第②步枪就是"侧躺着"的：竖直方向只剩枪的厚度（步枪 0.14 米），
而 0.64 米的枪身高度横在水平面上——看上去既不像握在手里，也分不清枪口朝哪。
滚 +90° 会变成"弹匣朝上"的倒置。**改完用 `RaidDemo/M7/渲染武器与特效预览图` 出图复核**，
别只看数字：包围盒只能告诉你"长宽高对不对"，看不出上下。

**挂点位置**：枪的枢轴在**握把**上，而枪托还在枢轴**后方 0.35 米**。
所以前向偏移必须大于 0.35 米才能让枪托离开躯干——灰盒阶段的 0.2 米是按"细长人形"定的，
换成 1.72 米高的卡通小人是把整段枪托留在身体里（P-47 的实机截图：
枪几乎全被躯干挡住，只露出一截枪托）。现在取 0.6 米，枪托后缘落在身体前缘之外；
持枪高度 1.05 米与服务器的枪口高度 `ServerRuntime.Combat.MuzzleHeight` 一致，
否则会出现"看着打中了却没命中"。

**弹道起点**：联机时服务器广播的 `WeaponFiredEvent.Origin` 是**服务器权威位置**上的枪口，
而本机是"本地预测 + 视觉跟随"，起步或变向时两者能差一米以上。
因此**自己那一发**的起点改用本地枪口（`PlayerWeaponView.MuzzleWorldPosition`），
终点仍用服务器的命中点；别人的弹道继续用服务器起点（他们的位置本来就只有服务器那一份）。

### 13.12 武器数值与"几枪击杀"（2026-09-16 定稿）

数值的真源在 **`ItemContentBuilder.Combat.cs` 的 `s_WeaponSpecs`**（资产是由它生成的），
下面这张表是当前值，改数值请改源表而不是只改资产——只改资产的话下次重建就被覆盖回去。

| 武器 | 单发伤害 | 射程 | 射速 | 弹匣 | 备注 |
| --- | --- | --- | --- | --- | --- |
| 手枪 PM | 18 | 8 米 | 400 发/分 | 8 | 起手武器，靠精度不靠火力 |
| **AK-74** | **35** | 12 米 | 600 发/分 | 30 | 主力步枪，躯干 4 枪 / 爆头 2 枪 |
| UZI 冲锋枪 | 13 | 10 米 | 900 发/分 | 25 | 与手枪同口径，靠射速与贴脸 |
| 泵动霰弹枪 | 11 × 6 颗弹丸（贴脸 66） | 6 米 | 75 发/分 | 6 | 一发定生死，打空代价最大 |

**敌人**：生命 100、护甲 2 级（护甲值 20 / 减伤 35% / 耐久 45）、备弹 90；
三种外观（红头盔 / 黄防化服 / 绿头盔士兵）共用同一份数值。

**AK-74 对 2 级甲敌人的逐枪算例**（弹药穿透力 5.45 = 15，公式见第 5 节）：

| 第几枪 | 有效护甲 | 穿透系数 | 实际伤害 | 累计/剩余 |
| --- | --- | --- | --- | --- |
| 1 | 20（满耐久） | 0 | 35 × 0.65 ≈ 22.8 | 剩 77.2；护甲耐久 45 → 32.8 |
| 2 | 14.6 | 0.02 | ≈ 23.1 | 剩 54.1；护甲 → 20.5 |
| 3 | 9.1 | 0.30 | ≈ 26.4 | 剩 27.7；护甲 → 8.3 |
| 4 | 3.7 | 0.57 | ≈ 29.7 | **击杀** |

无护甲目标 3 枪（35 × 3 = 105）、爆头 2 枪（暴击 ×2）、满甲爆头一枪打掉约 45 点——
即"打甲要先磨甲，破甲后明显变快"。

> **为什么值得专门记一笔**：AK 的伤害曾经是 **77**。那不是平衡值，而是 M10 做
> **资源热更端到端验证**时故意改上去的演示值（提交 `3497ca2`：25 → 77，为了让"热更后数值明显变化"
> 肉眼可见）。演示结束后它一直留在内容层，于是出现了"敌人被打两枪就死"的手感。
> 2026-09-16 回到 **35**，并把这条教训写进 `Docs/排障记录/`：**演示用的临时值必须当场记来源，否则会被当成设计值。**
