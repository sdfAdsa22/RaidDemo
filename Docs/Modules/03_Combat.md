...
> M8 批次 2 扩展：多弹丸武器（霰弹枪）与 12 号口径（2026-09-13）
...
| 编号 | 事项 | 结论 |
| --- | --- | --- |
| P-13 | **正式音效素材**与复杂开火 VFX | ✅ **M7 批次 3 已关闭**（2026-09-12）：枪声 / 脚步 / 打击 / 搜刮 / 撤离共 28 条加工音效、4 个粒子特效、玩家武器模型，见 [`07_音效与特效.md`](07_音效与特效.md)。灰盒阶段的程序化枪声**当时不在推迟范围内**——它是反馈信号而不是装饰，见 13.5；批次 3 起被正式素材取代（`WeaponAudioPlayer` 已删除） |
| P-14 ✅ | 多武器快捷切换 | **已完成（M3）**：滚轮上滚与数字键 2 为"下一把"，下滚与数字键 1 为"上一把"；只有两个武器槽时等价于主副互换。弹匣状态按武器分别保留 |
| P-15 | 弹道下坠与飞行时间 | 明确不做（见第 2 节设计前提） |
| P-16 | **多弹丸武器（霰弹枪）** | ✅ **M8 批次 2 已关闭**（2026-09-13）：`WeaponStats` 新增 `PelletCount` / `PelletSpreadDegrees`；一次扳机消耗一颗弹药、打出 N 条射线，弹丸在扇面内均匀排开；`WeaponFiredEvent` 新增 `PelletIndex`，枪声 / 枪口火焰 / 开火动画 / AI 噪声按 `PelletIndex == 0` 过滤，弹道与命中特效每颗都出。第一把多弹丸武器是泵动霰弹枪（6 颗、7° 扇面） |
...
- **战利品箱排除背包类物品**：背包占地 9 到 16 格，两件就能吃光整箱空间，
  把武器与弹药挤出去，而后者才是验证射击链路必需的东西。背包的获取途径已在 M6 由商人补齐。
